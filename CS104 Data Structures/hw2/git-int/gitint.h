...
//to track the files per commit
struct CommitFiles{
  CommitIdx index_;
  //map to store the state of the files per commit
  std::map<std::string, int> currentFiles_;

  CommitFiles(
    CommitIdx index_,
    std::map<std::string, int> currentFiles_) :
    index_(index_), currentFiles_(currentFiles_)
    {}
...
private:
     ...
     */
    std::map<std::string, int> buildState(CommitIdx from, CommitIdx to) const;
    ...
    std::set<std::string> stagedFiles_;
    std::map<std::string, CommitIdx> tag_;
    //key value is the commit index and the commitfiles are the files in that commit
    std::map<CommitIdx, CommitFiles> commit_files;
    //stores the most recent commit Index
    CommitIdx head_;
