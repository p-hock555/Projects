...
struct Square {
    ...
    // Data member: public
    //value_ can be only FREE WHITE OR BLACK
    SquareValue value_;
...
public:
    ...
    Board();
    Board(const Board& b);
    //deallocate all the memory on the heap
    ~Board();
    ...
    //accessor function to change the board_
    Board& flip_squares(char row, size_t column, Square::SquareValue turn);
    Board& flip_one(char row, size_t column, Square::SquareValue turn);
...
private:
    ...
    size_t player_count(Square::SquareValue turn);
