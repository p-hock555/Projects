...
protected:
    ...
    static Node<Key, Value>* predecessorHelper(Node<Key, Value>* current);
    int height(Node<Key, Value>* current) const;
    int max(int a, int b) const;
    bool isBalancedHelper(Node<Key,Value>* r) const;
...
void BinarySearchTree<Key, Value>::insert(const std::pair<const Key, Value> &keyValuePair)
{
    ...
    //p is the parent node and n is the current node we are using to traverse
    else{
      Node<Key, Value>* n = root_;
      Node<Key, Value>* p = root_;
      while(n != NULL)
      {
      ...
      Node<Key, Value>* newn = new Node<Key, Value>(keyValuePair.first, keyValuePair.second, p);
      if(p->getKey() < keyValuePair.first)
      {
        p->setRight(newn);
      }
      else{
...
{
    // TODO
    //first need a pointer to the node we want to delete
    Node<Key, Value>* n = root_;
...
BinarySearchTree<Key, Value>::predecessor(Node<Key, Value>* current)
{
    ...
    {
      return predecessorHelper(current->getLeft());
    }
...
BinarySearchTree<Key, Value>::successor(Node<Key, Value>* current)
{
    ...
    //we can go down the right
    if(current->getRight() != NULL)
    {
      return successorHelper(current->getRight());
    }
    //doesnt have a right pointer
    Node<Key, Value>* parent = current->getParent();
...
template<class Key, class Value>
Node<Key, Value>*
BinarySearchTree<Key, Value>::predecessorHelper(Node<Key, Value>* current)
{
  //TODO
  Node<Key, Value>* temp = current;
...
Node<Key, Value>* BinarySearchTree<Key, Value>::internalFind(const Key& key) const
{
    ...
    //if n is NULL it will return NULL anyway because we will exit the loop
    return n;
}
...
bool BinarySearchTree<Key, Value>::isBalanced() const
{
    ...
    return isBalancedHelper(r);
}

template<typename Key, typename Value>
bool BinarySearchTree<Key, Value>::isBalancedHelper(Node<Key,Value>* r) const
{
  int lh;
  int rh;
  ...
  lh = height(r->getLeft());
  rh = height(r->getRight());

  if (abs(lh - rh) <= 1 && isBalancedHelper(r->getLeft()) && isBalancedHelper(r->getRight())){
      return 1;
    }
...
int BinarySearchTree<Key, Value>::height(Node<Key, Value>* current) const{
  if (current == NULL){
    ...
    return 1 + max(height(current->getLeft()),
