...
private:
     ...
     *
     * @returns true if the vehicle should travel in the current row (east) and
     *          false if the vehicle should travel down the current column (south)
     */
    ...
    int** blockages_;//data struct to see where street is blocked
    int** indexes_;//keeps track of the indexes for the intersection history
    int** rowSeg_;//keep track of row segments
    int** colSeg_;//keep track of col segments 
    std::map<int, bool> intersectionHistory_;
    //int is the index of the tie, bool is true if they went row false if they went col
