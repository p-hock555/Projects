...
void lcd_writenibble(unsigned char lcdbits)
{
    ...
    PORTD &= ~(DATA_BITS); //clear the upper 4 bits of PORTD to 0;
    PORTD |= (lcdbits & DATA_BITS); //copy and set the upper 4 bits of x to PORTD upper 4 bits

    /* Make E signal (PB1) go to 1 and back to 0 */
    PORTB |= (1 << PB1);        // Set E to 1 for 125ns
    PORTB |= (1 << PB1);        // Make E longer for another 125ns
