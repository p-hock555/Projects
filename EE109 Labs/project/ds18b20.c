...
void ds_write1bit(void)
{
    ...
    DDRC |= (1<<PC3); //pull bus low for 2us
    _delay_us(2);
    DDRC &= ~(1<<PC3); //pull bus high for 60us
    _delay_us(60);
...
void ds_write0bit(void)
{
    ...
    // See page 15 and 16 of DS128B20 datasheet
    DDRC |= (1<<PC3); //pull the bus low for 60 us
    _delay_us(60); 
    DDRC &= ~(1<<PC3); //go high for 2 us
    _delay_us(2);
}
...
unsigned char ds_readbit(void)
{
    ...
    DDRC |= (1<<PC3); //pull bus low 2us
    _delay_us(2);
    DDRC &= ~(1<<PC3); //go high for 10us
    _delay_us(10);
    unsigned char x = (PINC & (1 << PC3)); //sample after 10us
    _delay_us(50); //delay the remainder of the time 62-12 = 50us
    return x; //return our sample
}
